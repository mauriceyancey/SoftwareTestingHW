...
public class CFG {
    ...
    public boolean isReachable(String methodFrom, String clazzFrom,
       ...
       try{
           JavaClass clazz = Repository.lookupClass(clazzFrom);
           clazz.getConstantPool();
           Method[] methods = clazz.getMethods();
           for(Method m : methods)
           {
               if(m.toString().contains(methodFrom))
               {
                   String methodToCheck = "invokestatic "+clazzTo +"."+ methodTo;
                   String methodCalls = m.getCode().toString().replace("\n", " ").replace("\t", " ");
                   if(methodCalls.contains(methodToCheck))
                   {
                       return true;
                   }
               }
       ...
       {
           System.out.println("Class " + clazzFrom + " not found");
       }
        return false;
    }
}
